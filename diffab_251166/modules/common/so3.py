...
class ApproxAngularDistribution(nn.Module):  # 近似角度的高斯分布.用球谐级数把"任意标准差的球面高斯"预计算成一张角度直方图，实现快速的角度采样
    ...
    def __init__(self, stddevs, std_threshold=0.1, num_bins=8192, num_iters=1024):
        ...
        self.mean = self.stddevs * 2
        self.std = self.stddevs

    @staticmethod
    def _pdf(x, e, L):  # x横轴；e标准差；L:迭代次数
        """
        Args:
            x:  (N, )
            e:  Float
        ...
        """
        x = x[:, None]  # (n_bins, *)
        c = ((1 - torch.cos(x)) / math.pi)  # (n_bins, *)
        l = torch.arange(0, L)[None, :]  # (*, L)
        a = (2 * l + 1) * torch.exp(-l * (l + 1) * (e ** 2))  # (*, L) 标准差e在这里起作用了
        b = (torch.sin((l + 0.5) * x) + 1e-6) / (torch.sin(x / 2) + 1e-6)  # (n_bins, L)

        f = (c * a * b).sum(dim=1)  # 标准差发挥作用 shape=(n_bins, )
        return f

    def _precompute_histograms(self):
        X, Y = [], []
        for std in self.stddevs:  # x每轮有一样，不一样的是y。y的具体值会受到标准差的变化【不同标准差，代表不同的球面高斯分布】
            std = std.item()
            x = torch.linspace(0, math.pi, self.num_bins)  # (n_bins,) [0-pi]均分为8192份(self.num_bins)
    ...
    def sample(self, std_idx):
        '''
        Args:
            std_idx:  Indices of standard deviation (integer).
        Returns:
            samples:  Angular samples [0, PI), same size as std.
        '''
        size = std_idx.size()
        std_idx = std_idx.flatten()  # (N*L,)

        prob = self.Y[std_idx]  # (L, n_bins)
    ...
    def sample_float(self, std_idx_float):
        """
        支持浮点索引的采样（使用线性插值）

        Args:
            std_idx_float:  Float indices of standard deviation, shape (N,).
        Returns:
            samples:  Angular samples [0, PI), same size as std.
        """
        idx_floor = torch.floor(std_idx_float).long().clamp(0, len(self.stddevs) - 1)
        idx_ceil = torch.ceil(std_idx_float).long().clamp(0, len(self.stddevs) - 1)
        frac = std_idx_float - idx_floor.float()

        samples_floor = self.sample(idx_floor)
        samples_ceil = self.sample(idx_ceil)

        samples = samples_floor * (1 - frac) + samples_ceil * frac

        return samples


def random_normal_so3(std_idx, angular_distrib, device='cpu', dtype=None):
    size = std_idx.size()  # (N, L)时间步：t*（N， L）
    u = F.normalize(torch.randn(list(size) + [3, ], device=device),
                    dim=-1)  # 得到一个单位球面数据(N, L, 3)。最初以高斯分布随机初始化，以行为单位归一化，即每个行向量模长变为1

...
def clampped_one_hot(x, num_classes):
    mask = (x >= 0) & (x < num_classes)  # (N, L) 未知的AA or padding
    x = x.clamp(min=0, max=num_classes - 1)  # 保证AA一定都是已知AA
    y = F.one_hot(x, num_classes) * mask[..., None]  # (N, L, C)  仅仅将真实AA转化为one-hot向量， padding或者原数据中AA未知的则全是0
    return y
...
class AngularEncoding(nn.Module):
    ...
    def __init__(self, num_funcs=3):
        super().__init__()
        self.num_funcs = num_funcs  # 3
        self.register_buffer('freq_bands', torch.FloatTensor(
            [i + 1 for i in range(num_funcs)] + [1. / (i + 1) for i in range(num_funcs)]
        ))  # [1,2,3, 1., 1/2, 1/3] 与模型参数（nn.Parameter）类似，但不参与梯度更新（即不会被优化器更新;用于存储需要与模型一起保存但不需要训练的张量（如均值、标准差等统计信息）。freq_bands 会成为模型的一个属性（如 model.freq_bands），并会随模型一起保存和加载。

    def get_out_dim(self, in_dim):
        return in_dim * (1 + 2 * 2 * self.num_funcs)  # z * (1 + 2*2*3)=z* 13=13z
    ...
    def forward(self, x):
        ...
        Args:
            x:  (..., d).
        """
        shape = list(x.shape[:-1]) + [-1]  # (N, L, 3, -1)
        x = x.unsqueeze(-1)  # (..., d, 1)
        code = torch.cat([x, torch.sin(x * self.freq_bands), torch.cos(x * self.freq_bands)], dim=-1)  # (..., d, 2f+1)
        code = code.reshape(shape)  # 每个角 都转化为 角度+sin+cos 形式
        return code
